...
public class PngCrushDirectoryWalker<E> extends DirectoryWalker<E> {
    ...
    public PngCrushDirectoryWalker(final Log log, boolean dryRun, String pngCrushExecutable, String arguments,
    ...
    protected void handleFile(File sourceFile, int depth, Collection<E> results) throws IOException {
        ...
        if (sourceFile.getName().endsWith(".png")) {
            ...
            // add arguments
            command.append(" ").append(getArguments());
            ...
            double gainedSizePercent = 100.0 - (100.0 * newSize / oldSize);

            final StringBuilder sbResult = new StringBuilder();
            sbResult.append(outputFileName).append(" : ");

            sbResult.append(oldSize + " => " + newSize);
            sbResult.append(", ").append(new DecimalFormat("#.##").format(gainedSizePercent)).append("%");

            totalGain += (oldSize-newSize);
            // if the new size is >= old size, image is already optimized
            if (newSize >= oldSize) {
                sbResult.append("  => OK ");
            } else if (gainedSizePercent > 5 || newSize - oldSize > 5000) {
                ...
                sbResult.append("  => Too small gain ");
            }
    ...
    public void walk() {
        try {
            ...
            log.info("Total gain : " + (totalGain/1024) + " ko");
        } catch (IOException e) {
